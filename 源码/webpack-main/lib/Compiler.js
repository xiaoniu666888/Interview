...
class Compiler {
	 ...
	 */
	// 注释
	// 1. 创建一个Watching实例，设置编译器为this，设置监听选项为watchOptions，设置回调函数为handler。
	 ...
	 */
	// 中文注释
	// 1. 运行编译器，设置运行状态为true，设置watchMode为false，调用run方法，开始编译。
	run(callback) {
		// 2. 判断是否正在运行，如果是，则抛出异常。
		if (this.running) {
			// 3. 返回一个错误。
		...
		const run = () => {
			// 调用beforeRun和run钩子，分别用于执行编译前的准备工作和编译逻辑。
			this.hooks.beforeRun.callAsync(this, err => {
				if (err) return finalCallback(err);
				// 调用run钩子，用于执行编译逻辑。
				this.hooks.run.callAsync(this, err => {
					if (err) return finalCallback(err);
					// 作用在于，如果缓存为空，则执行编译逻辑，否则读取缓存。
					this.readRecords(err => {
						if (err) return finalCallback(err);
						// 调用compile方法，用于执行编译逻辑。
						this.compile(onCompiled);
					});
				});
			});
